...
setwd("~/Documents/Input October/1.1") 

######### Read in data #########

## Import constraint matrix (information on amount of nutrient per g food group)
constraintmatrix <- read_excel("Input 1.1.xlsx")

## Import constraint limits (information on nutrient constraints)
limitsdata <- read_excel("Nutrient dir 1.1.xlsx") #containing data frame with nutrient, constraint direction and constraint limit (rhs=right-hand side) 
...
constraintsdata <- constraintmatrix %>% 
  select("Foodgroup", 
         ...
         "Total ruminant meat, maximum, g" = "Ruminant meat",
         "Total pork, maximum, g" = "Pork",
         "Total other meat, maximum, g" = "Other meat"
         )

## Import realism constraints (information on baseline intake, mean and percentiles)
intakepercentiles <- read_excel("Realism 1.1.xlsx") 
...
Nut <- round(problem_1$xopt, digits=1) #New diet name
...
output_newdiet <- t(as.matrix(Nut)) %*% as.matrix(constraintsdata[,3:58])
...
cstr <- read_excel("Nutrient ref 1.1.xlsx")
...
nutnames <- c("Energy (MJ)",
         ...
         "Total white meat, maximum, g",
            "Total ruminant meat, maximum, g",
         "Total pork, maximum, g",
         "Total other meat, maximum, g" 
)
...
dietdeparture #Calculate % change from observed diet
ruminantmeatdeparture #Calculate % change in content of ruminant meat
redmeatdeparture #Calculate % change in content of red meat
totalmeatdeparture #Calculate % change in content of all meat
sink(file=NULL)

#Create excel sheet including all results
library("writexl")
write_xlsx(list("Nutrients" = const_result2, "Diet" = diet_result, "Diet departure" = dietframe, "Ruminant meat departure" = dietframe4, "Red meat departure" = dietframe2, "Total meat departure" = dietframe3), "~/Documents/Input October/1.1/1.1 Result.xlsx")
