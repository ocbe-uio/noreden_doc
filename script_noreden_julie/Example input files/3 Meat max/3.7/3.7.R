...
setwd("~/Documents/Input October/3.7") 

######### Read in data #########

## Import constraint matrix (information on amount of nutrient per g food group)
constraintmatrix <- read_excel("Input 3.7.xlsx")

## Import constraint limits (information on nutrient constraints)
limitsdata <- read_excel("Nutrient dir 3.7.xlsx") #containing data frame with nutrient, constraint direction and constraint limit (rhs=right-hand side) 
...
intakepercentiles <- read_excel("Realism 3.7.xlsx") 
...
cstr <- read_excel("Nutrient ref 3.7.xlsx")
...
sink(file="3.7 Meat reduction NutriHealthGHGE30.txt")
#Print out
...
library("writexl")
write_xlsx(list("Nutrients" = const_result2, "Diet" = diet_result, "Diet departure" = dietframe, "Ruminant meat departure" = dietframe4, "Red meat departure" = dietframe2, "Total meat departure" = dietframe3), "~/Documents/Input October/3.7/3.7 Result.xlsx")
