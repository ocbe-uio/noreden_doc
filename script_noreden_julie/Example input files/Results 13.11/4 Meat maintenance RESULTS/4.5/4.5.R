...
setwd("~/Documents/Input October/4.5") 

######### Read in data #########

## Import constraint matrix (information on amount of nutrient per g food group)
constraintmatrix <- read_excel("Input 4.5.xlsx")

## Import constraint limits (information on nutrient constraints)
limitsdata <- read_excel("Nutrient dir 4.5.xlsx") #containing data frame with nutrient, constraint direction and constraint limit (rhs=right-hand side) 
...
constraintsdata <- constraintmatrix %>% 
  select("Foodgroup", 
         ...
         "Total ruminant meat, minimum, g" = "Ruminant meat"
         )

## Import realism constraints (information on baseline intake, mean and percentiles)
intakepercentiles <- read_excel("Realism 4.5.xlsx") 
...
cstr <- read_excel("Nutrient ref 4.5.xlsx")
...
sink(file="4.5 Meat reduction NutriHealthGHGE20.txt")
#Print out
...
library("writexl")
write_xlsx(list("Nutrients" = const_result2, "Diet" = diet_result, "Diet departure" = dietframe, "Ruminant meat departure" = dietframe4, "Red meat departure" = dietframe2, "Total meat departure" = dietframe3), "~/Documents/Input October/4.5/4.5 Result.xlsx")
